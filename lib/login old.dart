...
class _LoginAppsState extends State<LoginApps> {
  ...
  Widget build(BuildContext context) {
    ...
    // ]);
    return Scaffold(
        resizeToAvoidBottomInset: false, body: portraitFormLogin(context));
  }

  SafeArea portraitFormLogin(BuildContext context) {
    return SafeArea(
      child: Column(
                    ...
                    child: InkWell(
                      onTap: () async {
                        ...
                        if (_formKey.currentState!.validate()) {
                          try {
                            var req =
                                await ApiLogin.fetchNotificationAuthentication(
                                    username.text, password.text);
                            if (req.status == "success") {
                              setState(() {
                                isLoginSuccess = true;
                              });
                              pref.setBool("is_login_success", isLoginSuccess!);

                              pref.setString("username", username.text);
                              pref.setString("kartuId", req.kartuId.toString());
                              showDialog(
                                  context: context,
                                  builder: (context) {
                                    return DialogBox(
